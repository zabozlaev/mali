...
class Mali extends Emitter {
  ...
  init (path, name, options) {
    this.name = name
    this.load = mu.isString(path) || (mu.isObject(path) && path.root && path.file)
    ...
    if (this.load) {
      ...
      names.forEach(n => {
        ...
        if (service) {
          ...
          methods.forEach(m => {
            this.methods[n][camelCase(m.name)] = m
          })
        }
      })
    } else if (mu.isObject(this.proto)) {
      let names = Object.keys(this.proto)
      if (mu.isString(name)) {
        names = [name]
      } else if (Array.isArray(name)) {
        names = intersection(name, names)
      ...
      forOwn(this.proto, (v, n) => {
        if (mu.isObject(v) && !mu.isFunction(v) && names.indexOf(n) >= 0) {
          this.services[n] = v
          this.middleware[n] = []
  ...
  use (service, name, ...fns) {
    if (mu.isFunction(service)) {
      forOwn(this.middleware, (v, sn) => {
        this.middleware[sn] = concat(v, service, fns)
      })
    } else if (isPlainObject(service)) {
      const testKey = Object.keys(service)[0]
      if (mu.isFunction(service[testKey]) || Array.isArray(service[testKey])) {
        const serviceName = Object.keys(this.services)[0]
        forOwn(service, (mw, mwName) => {
          if (mu.isFunction(mw)) {
      ...
      } else if (isPlainObject(service[testKey])) {
        forOwn(service, (def, serviceName) => {
          forOwn(def, (mw, mwName) => {
            if (mu.isFunction(mw)) {
              this.use(serviceName, mwName, mw)
            } else if (Array.isArray(mw)) {
              this.use(serviceName, mwName, ...mw)
      ...
      if (!mu.isString(name)) {
        fns.unshift(name)
        const sNames = Object.keys(this.services)
        if (sNames.indexOf(service) >= 0) {
          this.middleware[service] = concat(this.middleware[service], fns)
          return
        } else {
          name = serviceName
          serviceName = Object.keys(this.services)[0]
        }
      }
      ...
      this.handlers[serviceName][name] = concat(this.middleware[serviceName], fns)
    }
  }
  ...
  toJSON () {
    ...
    const props = pull(own, ...REMOVE_PROPS, ...EE_PROPS)
